...
// 5. Load data from backend with static fallback
async function loadMenuData() {
  ...
  try {
    ...
    if (items && items.length > 0) {
      ...
      ALL_ITEMS = fallbackMenu;
    }
  } catch (error) {
    console.error('Failed to load menu details from server database, using local fallback:', error);
    ALL_ITEMS = fallbackMenu;
  }

  COFFEE = ALL_ITEMS.filter(i => i.category === 'coffee');
  TEAS = ALL_ITEMS.filter(i => i.category === 'teas');
  FOOD = ALL_ITEMS.filter(i => i.category === 'food');
  renderMenu();
}
